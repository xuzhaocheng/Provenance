...
class PVControllerViewController<T: ResponderClient> : UIViewController, ControllerVC {
    ...
    var shouldShowMFiPlusCombo = false
        ...
        if system.shortName == "N64" || system.shortName == "PSX" {
            ...
            if PVControllerManager.shared.hasControllers && hasMFiPlus {
                PVControllerManager.shared.allLiveControllers.forEach({ (key, controller) in
                    ...
                    self.shouldShowMFiPlusCombo = true
                })
            }
    }
    ...
    @objc func controllerDidConnect(_ note: Notification?) {
        ...
        shouldShowMFiPlusCombo = true
        if !mFiPlusComboViewIsActive {
            showMFiPlusCombo()
...
//        let pressedButtonBorder = 0.0
    ...
    func setupMFiPlusViews(for controller: GCController) {
        ...
        let comboFrame = CGRect(x: (UIScreen.main.bounds.width / 2) - ((mFiPlusControllerTopView.image?.size.width)! / 2), y: safeAreaInsets.top + yPadding, width: (mFiPlusControllerTopView.image?.size.width)!, height: (mFiPlusControllerTopView.image?.size.height)!)
        ...
        mFiPlusHUDGradientLayer.colors = [UIColor.black.cgColor, UIColor.clear.cgColor]
...
//            mFiPlusHUDDPadRight?.alpha = 1.0
    ...
    func layoutMFiPlusCombo() {
        let yPadding: CGFloat = 54
        let frame = CGRect(x: (UIScreen.main.bounds.width / 2) - ((mFiPlusControllerTopView.image?.size.width)! / 2), y: safeAreaInsets.top + yPadding, width: (mFiPlusControllerTopView.image?.size.width)!, height: (mFiPlusControllerTopView.image?.size.height)!)
    ...
    func hideMFIPlusCombo() {
        UIView.animate(withDuration: 1.0, delay: 0.75, options: [.curveEaseInOut], animations: {
            self.mFiPlusControllerCombo.alpha = 0.0
            ...
            UIView.animate(withDuration: 0.5, delay: 0.0, options: [.curveEaseInOut], animations: {
                ...
                self.shouldShowMFiPlusCombo = false
            })
        })
    }
