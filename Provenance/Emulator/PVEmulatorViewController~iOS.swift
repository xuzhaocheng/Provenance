...
open class EmulatorActionController: DynamicsActionController<EmulatorActionCell, String, UICollectionReusableView, Void, UICollectionReusableView, Void> {
...
//		}
	...
	open override func createCancelView() -> UIView {
		...
		let button = cancelView.subviews.compactMap { return $0 as? UIButton }.first!
	...
	@objc func showMenu(_ sender: Any?) {
		...
		if let player1 = controllerManager.player1 {
			...
			if (player1.extendedGamepad != nil || wantsStartSelectInMenu) && !hideP1MenuActions {
				...
				actionSheet.addAction(Action( "P1 Start", style: .default, handler: { action in
					self.core.setPauseEmulation(false)
					self.isShowingMenu = false
		...
		if let player2 = controllerManager.player2 {
			if (player2.extendedGamepad != nil || wantsStartSelectInMenu) {
				actionSheet.addAction(Action( "P2 Start", style: .default, handler: { action in
					self.core.setPauseEmulation(false)
					self.isShowingMenu = false
		...
		if let swappableCore = core as? DiscSwappable, swappableCore.currentGameSupportsMultipleDiscs {
			actionSheet.addAction(Action( "Swap Disc", style: .default, handler: { action in
		...
		if let actionableCore = core as? CoreActions, let actions = actionableCore.coreActions {
			actions.forEach { coreAction in
				actionSheet.addAction(Action( coreAction.title, style: .default, handler: { action in
					DispatchQueue.main.asyncAfter(deadline: .now() + 0.1, execute: {
		...
		#if os(iOS)
		actionSheet.addAction(Action( "Save Screenshot", style: .default, handler: { action in
			self.perform(#selector(self.takeScreenshot), with: nil, afterDelay: 0.1)
		}))
		#endif
		actionSheet.addAction(Action( "Game Info", style: .default, handler: { action in
		...
		if core.supportsSaveStates {
			actionSheet.addAction(Action( "Save States", style: .default, handler: { action in
		...
		actionSheet.addAction(Action( quitTitle, style: shouldSave ? .default : .destructive, handler: { action in
			self.quit(optionallySave: false)
		}))

		// If save and quit is an option, add it last with different style
		if shouldSave {
			actionSheet.addAction(Action( "Save & Quit", style: .destructive, handler: { action in
